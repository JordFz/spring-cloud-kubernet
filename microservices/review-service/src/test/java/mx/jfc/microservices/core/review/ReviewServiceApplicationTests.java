...
import mx.jfc.microservices.core.review.persistence.ReviewRepository;
...
class ReviewServiceApplicationTests extends MySqlTestBase {
	...
	void getReviewsNotFound(){
		getAndVerifyReviewsByProductId("?productId=123", OK)
				.jsonPath("$.length()").isEqualTo(0);
	}

	@Test
	void getReviewsInvalidParameterNegative(){
		int productInvalidId = -1;
		getAndVerifyReviewsByProductId("?productId="+productInvalidId, UNPROCESSABLE_ENTITY)
				.jsonPath("$.path").isEqualTo("/review")
	...
	private WebTestClient.BodyContentSpec getAndVerifyReviewsByProductId (String prodcutIdQuery, HttpStatus expectedStatus){
		return client.get()
				.uri("/review" + prodcutIdQuery)
